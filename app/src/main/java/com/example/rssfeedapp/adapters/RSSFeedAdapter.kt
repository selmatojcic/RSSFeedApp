...
class RSSFeedAdapter(
    ...
    override fun onBindViewHolder(holder: RSSFeedViewHolder, position: Int) {
        ...
        holder.itemView.deleteButton.setOnClickListener {
            AlertDialog.Builder(holder.itemView.deleteButton.context)
                .setTitle("Delete")
                .setMessage("Are you sure you want to delete " + rssFeed.title + "?")
                .setPositiveButton("Yes") { dialog, _ ->
                    rssFeeds.removeAt(position)
                    onRSSFeedClickedListener.deleteRSSFeed(rssFeed)
                    notifyItemRemoved(position)
                    Toast.makeText(
                        holder.itemView.deleteButton.context,
                        "Deleted " + rssFeed.title,
                        Toast.LENGTH_SHORT
                    )
                        .show()
                    dialog.dismiss()
